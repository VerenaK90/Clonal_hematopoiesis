...
r = R(tumor_id+"Run_model_heme_WGS_data_2_branched_clones.R")
...
class ConstrainedPrior(random_variables.DistributionBase):
        ...
        def rvs(self, *args, **kwargs):
                while True:
                  ...
                  if (size1 > size2) & (np.log(10**size1*10**N)/(10**lambda_ss*(age - ts1)) <= 1) & (np.log(10**size2*10**N)/(10**lambda_ss*(age - ts2)) <= 1) & (np.log(10**size1*10**N)/(10**lambda_ss*(age-ts1)) >= 0) & (np.log(10**size2*10**N)/(10**lambda_ss*(age-ts2)) >= 0) :
        ...
        def pdf(self, x):
          ...
          if not((size1 > size2) & (np.log(10**size1*10**N)/(10**lambda_ss*(age - ts1)) <= 1) & (np.log(10**size2*10**N)/(10**lambda_ss*(age - ts2)) <= 1)& (np.log(10**size1*10**N)/(10**lambda_ss*(age-ts1)) >= 0) & (np.log(10**size2*10**N)/(10**lambda_ss*(age-ts2)) >= 0)):
            return 0.0
...
db = "sqlite:///" + "/Model_fits_2_clones/WGS_heme/" + tumor_id + "/Model_fit_branched.db" # adjust according to sample
