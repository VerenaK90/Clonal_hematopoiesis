...
#' Subset on a particular cell type
#' 
...
.subset.cell.type <- function(tree, cell.type){
  ...
  new.tree <- drop.tip(new.tree, dying.cells, trim.internal = T, collapse.singles = F)
...
#' Sample a random subset on the tree
#' 
#' @param tree object of class phylo; the current tree. Needs the following additional list elements: tip.class, specifying the cell type and sel.adv., specifying the selective advantage of each tip
...
#' Compute the VAF of a mutation in the population from the tree
#' 
#' @param tree object of class phylo; the current tree. Needs the following additional list elements: tip.class, specifying the cell type and sel.adv., specifying the selective advantage of each tip
...
#     if(length(mutations.this.node)==0){next}
#     children <- Descendants(tree, i, type="all")
...
#   }
#   ## then, add tips
#   vaf <- c(vaf, rep(1, sum(tree$edge.length[tree$edge[,2] %in% tree$tip.label])))
#   
...
get_vaf_from_tree <- function(tree){
  ...
  for(i in all.nodes){
    ...
    mutations.this.node <- tree$edge.length[which(tree$edge[,2]==i)]
    children <- getDescendants(tree, i)
...
#' Compute the number of mutations per tip cell
#' 
#' @param tree object of class phylo; the current tree. Needs the following additional list elements: tip.class, specifying the cell type and sel.adv., specifying the selective advantage of each tip
...
get_mutations_per_tip <- function(tree){
  ...
  for(i in all.tips){
    ancestors.this.node <- Ancestors(x = tree, node = i, "all")
...
#' Get a binary mutation matrix from the tree
#' 
#' @param tree object of class phylo; the current tree. Needs the following additional list elements: tip.class, specifying the cell type and sel.adv., specifying the selective advantage of each tip
#' @return a matrix with rows corresponding to mutations and columns to cells; entries are binaries of 0 or 1, indicating, repsectively, absence or presence of the mutation.
#' @export
