...
for(patient.id in patient.ids){
  ...
  for(mode in c("branched", "linear")){
    ...
    if(model.support.selection.2_clones[2,paste(patient.id, mode, sep="_")] > 15){
      p <- p + geom_ribbon(data = data.frame(x = unlist(hdinterval.selection.2clones[hdinterval.selection.2clones$Parameter=="clone_size_1",c("lower", "upper")]),
                                             ymin = c(0,0),
                                             ymax = c(max.y, max.y)), aes(x=2/x, ymin=ymin, ymax = ymax), inherit.aes = F, fill="darkblue", alpha = 0.5) + 
        geom_ribbon(data = data.frame(x = unlist(hdinterval.selection.2clones[hdinterval.selection.2clones$Parameter=="clone_size_2",c("lower", "upper")]),
                                             ymin = c(0,0),
                                             ymax = c(max.y, max.y)), aes(x=2/x, ymin=ymin, ymax = ymax), inherit.aes = F, fill="darkgreen", alpha = 0.5)
    }else if(model.support.selection.2_clones[1,paste(patient.id, mode, sep="_")] > 15){
