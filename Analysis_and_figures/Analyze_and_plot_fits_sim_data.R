...
# collect model support for selection (%)
model.support.selection <- matrix(NA, nrow=1, ncol=nrow(sample.info), dimnames = list(c("90"), sample.info$SampleID))
...
for(patient.id in sample.info$SampleID){
  ...
  # compute the size of the clone according to exponential growth
  fits$size_of_clone <- exp(10^fits$par_lambda_ss*(1-fits$par_s_absolute)*(age - fits$par_t_s_absolute))/10^fits$par_N

  fits$size_of_clone[fits$size_of_clone>1] <- 1


  ## for selection associated values, store only the parameters associated with a clone >= 0.05 VAF (=10% size)

  fits.selection <- fits[fits$size_of_clone >= 0.1, ]
  fits.neutral <- fits[fits$size_of_clone < 0.1, ]

  if(nrow(fits.selection)>0){
    fits.selection$growth_per_year <-  exp(10^fits.selection$par_lambda_ss*(1-fits.selection$par_s_absolute)*365)-1
    hdinterval.selection <- as.data.frame(t(hdi(fits.selection, credMass=0.8)))
    hdinterval.selection$Parameter <- rownames(hdinterval.selection)
    hdinterval.selection$Median <- apply(fits.selection, 2, function(x){median(as.numeric(x))})
    hdinterval.selection$Sample <- patient.id
  ...
  if(nrow(fits.neutral)>0){
    hdinterval.neutral <- as.data.frame(t(hdi(fits.neutral, credMass=0.8)))
    hdinterval.neutral$Parameter <- rownames(hdinterval.neutral)
    hdinterval.neutral$Median <- apply(fits.neutral, 2, function(x){median(as.numeric(x))})
    hdinterval.neutral$Sample <- patient.id
    hdinterval.neutral$Depth <- depth
    neutral.parameters <- rbind(neutral.parameters, hdinterval.neutral)
  }
  ## condition on median clone size >= 0.1
  model.support.selection[1,patient.id] <- nrow(fits.selection)/10

}

# plot mean and average of conditional posterior probability for different clone sizes:
...
to.plot.selection <- to.plot[to.plot$Depth==90 & !is.na(to.plot$`Posterior probability`) &
                               to.plot$Patient %in% c("STN8", "STS25", "STS39", "STS46", "STS53", "STS84"),]

ggplot(to.plot.selection,
       aes(x=Patient, y=`Posterior probability`, fill=Clone_size)) + geom_col(width=0.5, col=NA) +
