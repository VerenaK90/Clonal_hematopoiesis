...
for(patient.id in patient.ids){
  ...
  ###### load observed data
  sort <- "CD34"
  directory <- paste0(analysis.directory, "/data/Model_fits/WGS_heme/", paste(patient.id, sort, "nsc", sep="_"))
  ...
  driver.information <- putative.drivers[,c("FORMAT", "CHROM", "POS", "REF", "ALT", "GENE", "AAchange",
                                            paste(patient.id, sort.type, sep="_"))]
  ...
  if(any(grepl(paste0(patient.id, "_"), colnames(driver.information)))){
    driver.information$VAF <- Extract.info.from.vcf(driver.information, info="VAF", type="snvs", mutationcaller="mpileup", sample.col.mpileup =  paste(patient.id, sort.type, sep="_"))
    driver.information$Depth <- Extract.info.from.vcf(driver.information, info="depth", type="snvs", mutationcaller="mpileup", sample.col.mpileup =  paste(patient.id, sort.type, sep="_"))
...
####################################################################################################################################################
## Extended Data. Fig. 7e: plot the model fit

pdf(paste0(analysis.directory, "/Figures/Ext_Data_Fig_7e.pdf"), width=3.5, height=3.5)

print(plotlist.model.vs.data[["T2 CD34+"]])
